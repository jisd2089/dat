...
type CallBack struct {
	SuccessCallBack   func(chan *ChangeEvent, *Repository, []byte) (interface{}, error)
...
func request(requestUrl string, connectionConfig *ConnectConfig, callBack *CallBack, notifyChan chan *ChangeEvent, repository *Repository) (interface{}, error) {
	...
	for {
		...
		if res == nil || err != nil {
			logger.Error("Connect Apollo Server Fail,Error1:", err)
			continue
		}
		...
		case http.StatusOK:
			...
			if err != nil {
				logger.Error("Connect Apollo Server Fail,Error2:", err)
				continue
			}

			if callBack != nil && callBack.SuccessCallBack != nil {
				return callBack.SuccessCallBack(notifyChan, repository, responseBody)
			} else {
				return nil, nil
			}
		...
		default:
			logger.Error("Connect Apollo Server Fail,Error3:", err)
			if res != nil {
...
func requestRecovery(notifyChan chan *ChangeEvent, appConfig *AppConfig, repository *Repository,
	...
	for {
		...
		response, err = request(requestUrl, connectConfig, callBack, notifyChan, repository)
		if err == nil {
			return response, err
		}
