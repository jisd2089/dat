...
type RespDetail struct {
	...
	Cons_type_amtrate      float64 `json:"consvariety"`            // 消费金额最大的类别占总消费金额的比值
	...
	Consume_12m_cnt        float64 `json:"consume_12m_cnt"`        // 最近12个月电商消费笔数
	Type_12m_sum           float64 `json:"type_12m_sum"`           // 最近12个月的累计购买产品种类数
	Level_12m_consume      float64 `json:"level_12m_consume"`      // 最近12个月的消费档次
	Month_num              float64 `json:"month_num"`              // 最近12个月有消费记录的月份数
	Mon_max_record         float64 `json:"mon_max_record"`         // 最近12个月单月最大消费金额
	Latest_record_m        float64 `json:"latest_record_m"`        // 最近消费月份距今月份数
	Max_interval_month     float64 `json:"max_interval_month"`     // 最大无消费间隔月数
	Cos_stab               float64 `json:"cos_stab"`               // 近12个月的消费稳定度
	Label_24_num           float64 `json:"label_24_num"`           // 近两年的消费类别个数
	...
	Child_6m_num           float64 `json:"child_6m_num"`           // 近6个月孩童类（儿童，婴儿，产妇类）消费笔数
	Car_6m_money           float64 `json:"car_6m_money"`           // 近6个月汽车类消费金额
	Car_6m_num             float64 `json:"car_6m_num"`             // 近6个月汽车类消费笔数
	Diamond_6m_money       float64 `json:"diamond_6m_money"`       // 近6个月装饰（手表,饰品，宝石）类消费金额
	Diamond_6m_num         float64 `json:"diamond_6m_num"`         // 近6个月装饰（手表,饰品，宝石）类消费笔数
	Sports_6m_money        float64 `json:"sports_6m_money"`        // 近6个月运动类消费金额
	Sports_6m_num          float64 `json:"sports_6m_num"`          // 近6个月运动类消费笔数
	Entertainment_6m_money float64 `json:"entertainment_6m_money"` // 近6个月文娱类（书籍，乐器，音乐，影视）消费金额
	Entertainment_6m_num   float64 `json:"entertainment_6m_num"`   // 近6个月文娱类（书籍，乐器，音乐，影视）消费笔数
	Digital_6m_money       float64 `json:"digital_6m_money"`       // 近6个月3C类消费金额
	Digital_6m_num         float64 `json:"digital_6m_num"`         // 近6个月3C类消费笔数
	Virtual_6m_money       float64 `json:"virtual_6m_money"`       // 近6个月虚拟商品类消费金额
	Virtual_6m_num         float64 `json:"virtual_6m_num"`         // 近6个月虚拟商品类消费笔数
	Hosehold_6m_money      float64 `json:"hosehold_6m_money"`      // 近6个月家用电器类商品总消费金额
	Hosehold_6m_num        float64 `json:"hosehold_6m_num"`        // 近6个月家用电器类商品总消费笔数
	Phone                  float64 `json:"phone"`                  // 手机号
}
